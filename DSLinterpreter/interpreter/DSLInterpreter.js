...
// [[SYNTAX PARSER]]
// should design a syntax checker to validate the syntax of each step before processing it
// this will help catch errors early and provide feedback to the user
//
...
for (let i = 0; i < TestBlocks.length; i++) {
    ...
    for (let j = 0; j < logicArray.length; j++) {
        ...
        if(action.toLowerCase() == "call"){
            console.log("concatenating called test block:"+target);
            for (let k = 0; k < DispatchPackets[target].length; k++) {
                console.log(`adding step from ${target} called test block:`+JSON.stringify(DispatchPackets[target][k]));
                
                DispatchPacket.push(DispatchPackets[target][k]);
            }
            continue; 
        }
